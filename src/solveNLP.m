...
%         end
        ...
        funcs.objective         = @(x)data.funcs.objective(x,data);
        funcs.constraints       = @(x)data.funcs.constraints(x,data);
        funcs.gradient          = @(x)data.funcs.gradient(x,data);
        funcs.jacobian          = @(x)data.funcs.jacobian(x,data);
        funcs.hessian           = @(x,lambda_C)data.funcs.hessian(x,1,lambda_C,data);

        
        tA=tic;
        [z, info] = LISSQP_v9(funcs,[],bounds, qpSolver, options, NLP.z0);
        tB=toc(tA);
