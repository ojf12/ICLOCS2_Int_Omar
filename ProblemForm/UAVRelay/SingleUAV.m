...
Q_init = -v_avg*(20*60);
source(1) = Q_init*(8/10);
source(2) = Q_init*(6/10);
...
problem.states.x0=[v_avg, Q_init];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[v_avg, Q_init];
problem.states.x0u=[v_avg, Q_init];

% State bounds. xl=< x <=xu
problem.states.xl=[v_min, Q_init];
problem.states.xu=[v_max, 0];
...
% State error bounds
problem.states.xErrorTol=[1 5]; 

% State constraint error bounds
problem.states.xConstraintTol=[0.5 5];
problem.states.xrConstraintTol=[0.5 5];

% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[v_min, 0]; 
problem.states.xfu=[v_max, 0];
...
% Input bounds
problem.inputs.ul=[0,    -inf];
problem.inputs.uu=[P_max, inf];

% Bounds on the first control action
problem.inputs.u0l=[0, -inf];
problem.inputs.u0u=[P_max, inf];

% Input rate bounds
problem.inputs.url=[-inf -5];
problem.inputs.uru=[ inf 5];

% Input constraint error bounds
problem.inputs.uConstraintTol  = [1 1];
problem.inputs.urConstraintTol = [1 1];

% Guess the input sequences with [u0 ... uf]
guess.inputs = [];

% Bounds for path constraint function gl =< g(x,u,p,t) =< gu
problem.constraints.gl=[-inf, F_min];
problem.constraints.gu=[-r_Des,    F_max];

% Path constraint error bounds
problem.constraints.gTol=[ 1 6];
...
problem.data.auxdata=[B, K, alpha, alt, ss, P_max, M, mass, D_scaling, c_1, c_2, source(1), source(2)];
...
c_1     = data.auxdata(10);
c_2     = data.auxdata(11);
source(1)  = data.auxdata(12);
source(2)  = data.auxdata(13);
...
c(:,1)= - (B*scaling)*log2(1 + (K.*p1)./(ss.*((q).^2 + alt^2).^alpha));

% Newtonian force constraint
c(:,2)=  mass*a + Drag(v,c_1,c_2);
