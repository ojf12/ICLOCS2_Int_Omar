function [problem,guess] = UAVMultipleAccessComms
...
v_const = 20;
Q_init  = -v_const*(20*60)/2;
Q_final =  v_const*(20*60)/2;
% Delta, UAV U_2 lateral displacement
delta = 500;
...
problem.states.x0=[D_init, D_init];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[D_init, D_init];
problem.states.x0u=[D_init, D_init];

% State bounds. xl=< x <=xu
problem.states.xl=[0,      0];
problem.states.xu=[D_init, D_init];

% State rate bounds. xrl=< x <=xru
problem.states.xrl=[-inf -inf]; 
problem.states.xru=[inf   inf]; 

% State error bounds
problem.states.xErrorTol=[1 1]; 

% State constraint error bounds
problem.states.xConstraintTol=[1 1];
problem.states.xrConstraintTol=[1 1];
...
% Input bounds
problem.inputs.ul=[0,       0,      0,      0];
problem.inputs.uu=[P_max,   inf,    P_max,  inf];

% Bounds on the first control action
problem.inputs.u0l=[0,       0,      0,      0];
problem.inputs.u0u=[P_max,   inf,    P_max,  inf];

% Input rate bounds
problem.inputs.url=[-inf -inf -inf -inf];
problem.inputs.uru=[ inf  inf  inf  inf];

% Input constraint error bounds
problem.inputs.uConstraintTol=[1 1 1 1];
problem.inputs.urConstraintTol=[1 1 1 1];

% Guess the input sequences with [u0 ... uf]
guess.inputs = [];

% Bounds for path constraint function gl =< g(x,u,p,t) =< gu
problem.constraints.gl=[-0,   -0,   -0];
problem.constraints.gu=[0,      0,      0];

% Path constraint error bounds
problem.constraints.gTol=[1 1 1];
...
problem.data.auxdata=[B, K, alpha, alt, ss, P_max, D_scaling, Q_init, v_const, delta];
...
Q_init  = data.auxdata(8);
v_const = data.auxdata(9);
delta   = data.auxdata(10);
...
q = Q_init + v_const*t;
dist_U1 = (q).^2 + alt^2;
dist_U2 = (q).^2 + alt^2 + delta.^2;

Kappa1 = (K/ss)./ ((dist_U1).^alpha);
Kappa2 = (K/ss)./ ((dist_U2).^alpha);

% Mac uplink constraints to relay
c(:,1)= r1 -      (B*scaling)*log2(1 + p1.*Kappa1);
c(:,2)= r2 -      (B*scaling)*log2(1 + p2.*Kappa2);
c(:,3)= r1 + r2 - (B*scaling)*log2(1 + p1.*Kappa1 + p2.*Kappa2);
%------------- END OF CODE --------------
