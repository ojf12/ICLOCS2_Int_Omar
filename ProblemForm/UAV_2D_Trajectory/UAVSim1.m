...
X_init = -10000; Y_init = -5000;
X_final = 10000; Y_final = -5000;
q_init = ((X_init-x_s)^2 + (Y_init-y_s)^2 + alt^2);
% Bounds
Xb = [-inf inf];
Yb = [-inf inf];
psib = [-inf,inf];
...
problem.states.x0=[D_init, X_init, Y_init, psi_init, w_init];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[D_init, X_init, Y_init, psi_init, w_init];
problem.states.x0u=[D_init, X_init, Y_init, psi_init, w_init];

% State bounds. xl=< x <=xu
problem.states.xl=[0,      Xb(1), Yb(1), psib(1), wb(1)];
problem.states.xu=[D_init, Xb(2), Yb(2), psib(2), wb(2)];

% State rate bounds. xrl=< x <=xru
problem.states.xrl=[-inf -inf -inf -inf -0.1]; 
problem.states.xru=[ inf -inf  inf  inf 0.1 ]; 

% State error bounds
problem.states.xErrorTol=[1 1 1 1 1]; 

% State constraint error bounds
problem.states.xConstraintTol = [1 1 1 1 1];
problem.states.xrConstraintTol= [1 1 1 1 1];

% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[0, X_final, Y_final, psi_init, 0]; 
problem.states.xfu=[0, X_final, Y_final, psi_init, 0];
...
% Input bounds
problem.inputs.ul=[0,     0,   wb(1), 0];
problem.inputs.uu=[P_max, inf, wb(2), inf];

% Bounds on the first control action
problem.inputs.u0l=[0,     0,   wb(1), q_init];
problem.inputs.u0u=[P_max, inf, wb(2), q_init];


% Input rate bounds
problem.inputs.url=[-inf -inf -0.2 -inf];
problem.inputs.uru=[ inf  inf  0.2 inf];
...
problem.constraints.gl=[-inf];
problem.constraints.gu=[0 ];

% Path constraint error bounds
problem.constraints.gTol=[ 1];
...
cost_a = data.auxdata(11);

% Define relevant input variables
p1 = u(:,1);
a  = u(:,3);

stageCost = p1;%+ cost_a*a;
...
c(:,1) = r1  - B*log(1 + (K.*p1)./(ss.*((x1-x_s).^2 + (y1-y_s).^2 + alt^2).^alpha));
