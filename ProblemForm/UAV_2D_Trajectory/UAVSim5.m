...
problem.states.x0=[X_init, Y_init, psi_init, w_init, 0];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[X_init, Y_init, psi_init, w_init, 0];
problem.states.x0u=[X_init, Y_init, psi_init, w_init, 0];

% State bounds. xl=< x <=xu
problem.states.xl=[Xb(1), Yb(1), psib(1), wb(1), 0];
problem.states.xu=[Xb(2), Yb(2), psib(2), wb(2), inf];

% State rate bounds. xrl=< x <=xru
problem.states.xrl=[-inf -inf -inf -0.1, -inf]; 
problem.states.xru=[-inf  inf  inf 0.1, inf]; 
...
problem.states.xfl=[X_final, Y_final, psi_init, 0, 0]; 
problem.states.xfu=[X_final, Y_final, psi_init, 0, inf];
...
% Input bounds
problem.inputs.ul=[wb(1), 0, 0];
problem.inputs.uu=[wb(2), inf, 1];

% Bounds on the first control action
problem.inputs.u0l=[wb(1), 0, 0];
problem.inputs.u0u=[wb(2), inf, 1];


% Input rate bounds
problem.inputs.url=[-0.2 -inf, -inf ];
problem.inputs.uru=[0.2 inf, inf ];
...
%c = [];
c(:,1) = lambda1.*((y1+1000).^2 + (x1).^2 - 50^2);
