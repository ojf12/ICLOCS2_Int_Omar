...
problem.states.x0=[D_init, X_init, Y_init, psi_init, w_init,V];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[D_init, X_init, Y_init, psi_init, w_init, V];
problem.states.x0u=[D_init, X_init, Y_init, psi_init, w_init, V];

% State bounds. xl=< x <=xu
problem.states.xl=[0,      Xb(1), Yb(1), psib(1), wb(1), Vb(1)];
problem.states.xu=[D_init, Xb(2), Yb(2), psib(2), wb(2), Vb(2)];

% State rate bounds. xrl=< x <=xru
problem.states.xrl=[-inf -inf -inf -inf -0.1 -inf]; 
problem.states.xru=[ inf -inf  inf  inf 0.1 inf]; 
...
problem.states.xfl=[0, X_final, Y_final, psi_init, 0, Vb(1)]; 
problem.states.xfu=[0, X_final, Y_final, psi_init, 0, Vb(2)];
...
% (5): acceleration of UAV 

% Input bounds
problem.inputs.ul=[0,     0,   wb(1), 0,   ab(1)];
problem.inputs.uu=[P_max, inf, wb(2), inf, ab(2)];

% Bounds on the first control action
problem.inputs.u0l=[0,     0,   wb(1), q_init, 0];
problem.inputs.u0u=[P_max, inf, wb(2), q_init, 0];


% Input rate bounds
problem.inputs.url=[-inf -inf -0.2 -inf -0.2];
problem.inputs.uru=[ inf  inf  0.2 inf  0.2];
